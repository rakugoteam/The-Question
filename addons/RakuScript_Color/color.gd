...
func _on_editor_script_changed(script: Script):
	if !script: return
	if script.resource_path.get_extension() == "rk":
		var current_editor: CodeEdit = script_editor.get_current_editor().get_base_editor()
		current_editor.syntax_highlighter = dialogue_highlighter
		pass
