...
func test_ask_ui():
	var ask_panel := add_from_scene(ask_panel_scene) as AskPopup
	assert_not_null(ask_panel)
	ask_panel.say_panel = add_from_scene(say_panel_scene)
	assert_dialogue_panel(ask_panel.say_panel)
	watch_rakugo_signals()
	await wait_parse_and_execute_script(file_path)
	
	await wait_ask({"name": "Test"}, "Are you human ?", "Yes")
	assert_dialogue_panel_text(ask_panel.say_panel, "# Test\n", "Are you human ?")
	...
	assert_dialogue_panel_text(ask_panel.say_panel, "", "Your answer was No ?")
